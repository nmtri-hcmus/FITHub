    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div className="flex bg-gray-800/80 p-1.5 rounded-2xl border border-gray-700/60">
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-sm font-semibold transition-all ${
              activeTab === 'COMMUNITY'
                ? 'bg-surface text-white shadow-md shadow-black/5'
        ...
        <button
          ...
          className="bg-primary text-white font-bold py-3 px-6 rounded-xl hover:bg-primary-light transition-colors flex items-center gap-2 border border-primary-dark/20"
        >
      ...
      <div className="mb-6 space-y-3">
        <div className="relative">
          <svg className="absolute left-4 top-3.5 w-4 h-4 text-text-subtle" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <input
            ...
            className="w-full bg-surface border border-surface-edge rounded-xl pl-10 pr-4 py-3 text-sm text-white focus:outline-none focus:border-primary shadow-sm"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery('')}
              className="absolute right-3 top-3 p-0.5 text-text-subtle hover:text-gray-300 transition-colors"
            >
        ...
        <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              className={`text-xs font-semibold px-4 py-1.5 rounded-full border transition-all ${
                activeFilter === f
                  ...
                  : 'bg-surface text-text-muted border-surface-edge hover:border-gray-700 hover:text-white'
              }`}
            >
              {FILTER_LABELS[f]}
            </button>
          ...
          {filteredRecipes.length !== baseRecipes.length && (
            <span className="text-xs text-text-subtle self-center ml-1">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="h-64 bg-surface rounded-2xl border border-surface-edge animate-pulse" />
          ))}
        </div>
      ) : filteredRecipes.length === 0 ? (
        <div className="text-center py-20 bg-surface rounded-[2rem] border border-surface-edge shadow-sm">
          <div className="w-20 h-20 bg-surface-edge rounded-full flex items-center justify-center mx-auto mb-6">
            <svg className="w-10 h-10 text-text-subtle" fill="none" viewBox="0 0 24 24" stroke="currentColor">
