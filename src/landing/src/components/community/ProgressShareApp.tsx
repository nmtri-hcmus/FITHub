...
const Toggle: React.FC<{ label: string; checked: boolean; onChange: (v: boolean) => void; disabled?: boolean }> = ({ label, checked, onChange, disabled }) => (
  <label className={`flex items-center justify-between p-3 rounded-xl border ${checked ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 bg-white'} ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:border-emerald-300'} transition-all`}>
    <span className={`text-sm font-bold ${checked ? 'text-emerald-900' : 'text-gray-700'}`}>{label}</span>
    <div className={`relative w-11 h-6 rounded-full transition-colors ${checked ? 'bg-emerald-500' : 'bg-gray-300'}`}>
      <div className={`absolute top-1 bg-white w-4 h-4 rounded-full transition-all ${checked ? 'left-6' : 'left-1'}`} />
    </div>
...
const FormatBtn: React.FC<{ label: string; active: boolean; onClick: () => void; icon: string }> = ({ label, active, onClick, icon }) => (
  <button
    onClick={onClick}
    className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold transition-all ${active ? 'bg-black text-white shadow-md' : 'text-gray-500 hover:text-black hover:bg-gray-100'}`}
  >
    <span>{icon}</span>
...
export const ProgressShareApp: React.FC = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row bg-gray-50 text-black" style={{ minHeight: 'calc(100vh - 80px)' }}>

      {/* ── Left Panel: Controls ── */}
      <div className="w-full lg:w-72 bg-white border-r border-gray-200 p-6 overflow-y-auto shrink-0 shadow-sm">
        <h2 className="text-xl font-black mb-1">Progress Studio</h2>
        <p className="text-xs text-gray-400 mb-6">Generate a shareable graphic of your achievements.</p>

        <div className="space-y-6">
          {/* Metrics */}
          <div>
            <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Metrics to Display</h3>
          ...
          {/* Visuals */}
          <div>
            <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Visuals</h3>
            <div className="space-y-2">
              ...
              {showPhoto && (
                <div className="ml-1 mt-2">
                  ...
                  <button
                    ...
                    className="w-full py-2 border-2 border-dashed border-gray-300 rounded-xl text-xs font-bold text-gray-500 hover:border-emerald-500 hover:text-emerald-600 hover:bg-emerald-50 transition-colors"
                  >
        ...
        <div className="mt-8">
          ...
          >
            <span>✨</span> Generate &amp; Share
          </button>
        </div>
      </div>

      {/* ── Center: Canvas Preview ── */}
      <div className="flex-1 flex flex-col items-center justify-center p-6 bg-gray-100/60 overflow-auto">
        {/* Format switcher */}
        <div className="flex bg-white border border-gray-200 rounded-full p-1 mb-6 shadow-sm gap-1">
          <FormatBtn label="Square Feed" active={format === 'square'} onClick={() => setFormat('square')} icon="📱" />
          <FormatBtn label="Vertical Story" active={format === 'vertical'} onClick={() => setFormat('vertical')} icon="🎞️" />
          <FormatBtn label="Landscape" active={format === 'landscape'} onClick={() => setFormat('landscape')} icon="🖼️" />
        </div>
          ...
          <div className="relative z-20 h-full w-full p-7 flex flex-col">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-gray-200 overflow-hidden border-2 border-white/30 flex-shrink-0">
            ...
            <div className={`flex ${format === 'landscape' ? 'flex-row gap-4 items-end' : 'flex-col gap-4'} mt-auto mb-4`}>
              {showWeight && (
                <div className="bg-white/10 backdrop-blur-sm border border-white/10 rounded-2xl p-4 flex-1">
                  <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-2">Weight Journey</p>
                  <div className="flex items-end gap-3">
                    ...
                    <div className="flex items-baseline gap-1">
                      ...
                      <span className="text-xs text-emerald-400 font-bold">-12 lbs ↓</span>
                    </div>
                  </div>
                </div>
              )}
              {showMacros && (
                <div className="bg-white/10 backdrop-blur-sm border border-white/10 rounded-2xl p-4 flex-1 flex items-center gap-3">
                  <div
                  ...
                  <div>
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1">Daily Macros</p>
            ...
            <div className="flex items-center justify-center gap-1.5 pt-3 border-t border-white/10">
              ...
              <span className="text-[9px] text-gray-400 font-medium ml-1.5">fithub.app</span>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/40 backdrop-blur-sm" onClick={() => setIsExportOpen(false)}>
          ...
          >
            <div className="p-5 border-b border-gray-200 flex items-center justify-between">
              <div>
                <h2 className="text-lg font-black text-black">Export Your Card</h2>
                <p className="text-xs text-gray-400 mt-0.5">UC-19: Share Progress Milestone</p>
              </div>
              <button onClick={() => setIsExportOpen(false)} className="text-gray-400 hover:text-black text-2xl leading-none w-8 h-8 flex items-center justify-center">&times;</button>
            </div>
            ...
            <div className="p-6 space-y-3 flex-1">
              ...
              <button
                onClick={handleCopy}
                disabled={generating}
                className="w-full flex items-center gap-4 bg-gray-100 text-black px-5 py-4 rounded-2xl font-bold hover:bg-gray-200 transition-colors disabled:opacity-50"
              >
                <span className="text-2xl">📋</span>
                <div className="text-left">
                  <div className="text-sm font-black">Copy to Clipboard</div>
                  <div className="text-xs font-normal text-gray-500">Paste into Instagram, Twitter, etc.</div>
                </div>
              </button>

              <div className="border-t border-gray-100 pt-4">
                <p className="text-xs text-center text-gray-400">
                  Share your progress to Instagram, X (Twitter), Facebook, or any platform — just paste the copied image!
                </p>
              </div>
            </div>
            ...
            {generating && (
              <div className="p-5 bg-gray-50 border-t border-gray-100 flex items-center gap-3">
                ...
                <p className="text-sm font-medium text-gray-600">Generating your card...</p>
              </div>
            )}
          </div>
        </div>
