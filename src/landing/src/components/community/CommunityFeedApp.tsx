    ...
    <div className="flex h-full w-full max-w-7xl mx-auto">
      ...
      <div className="flex-1 border-r border-gray-200 min-h-screen">
        {activeView === 'vote_meal' ? (
          ...
          <>
        {/* Page header — Forum title OR Sub-Community banner */}
        {activeGroup ? (
          /* Sub-Community Banner */
          <div className="relative h-48 w-full bg-gray-200 overflow-hidden flex flex-col justify-end">
            <img
            ...
            <div className="relative z-10 flex justify-between items-end p-6">
              ...
              <button
                ...
                className="bg-white/10 hover:bg-white/20 text-white border border-white/20 backdrop-blur-md px-4 py-2 rounded-xl text-xs font-bold transition-all"
              >
                ← Back to Feed
              </button>
            </div>
            ...
            <div 
              ...
              className="px-6 py-6 border-b border-gray-100 bg-emerald-500 hover:bg-emerald-600 transition-colors cursor-pointer text-white flex items-center justify-between"
            >
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-xl bg-white/20 flex items-center justify-center text-white text-xl font-black shrink-0">
                  🏆
                </div>
                <div>
        ...
        {/* Header Area */}
        <div className="p-6 border-b border-gray-200 bg-white sticky top-[80px] z-20 shadow-sm">
      ...
      <div className="hidden lg:block w-80 shrink-0 p-6">
        <div className="bg-white border border-gray-200 rounded-2xl p-5 mb-6 shadow-sm">
          <h3 className="font-bold text-gray-900 mb-4 uppercase text-xs tracking-wider">Popular Topics</h3>
          <div className="space-y-3">
            {['#MealPrepSunday', '#FormCheck', '#BeginnerGains'].map(tag => (
              <a key={tag} href="#" className="block text-sm font-semibold text-gray-700 hover:text-emerald-600 transition-colors">
                {tag}
              </a>
            ))}
          </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
          <h3 className="font-bold text-gray-900 mb-4 uppercase text-xs tracking-wider">Featured Coaches</h3>
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-gray-900">Alex Jenkins</p>
                <p className="text-xs text-gray-500">Strength &amp; Conditioning</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {activeGroup && (
          <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 shadow-sm mt-6">
            <h3 className="font-bold text-emerald-800 mb-2 text-sm">📌 Posting in this group</h3>
            <p className="text-xs text-emerald-700">Posts in <strong>{activeGroup.name}</strong> are reviewed by admins before they appear publicly.</p>
          </div>
        )}
      </div>
...
const PostCard: React.FC<{ post: BackendPost; onReport: () => void }> = ({ post, onReport }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {isCoach ? (
                <span className="bg-emerald-100 text-emerald-800 text-[10px] font-black uppercase px-2 py-0.5 rounded-full tracking-wide">
                  Verified Coach
                </span>
              ) : (
                <span className="bg-gray-100 text-gray-600 text-[10px] font-bold uppercase px-2 py-0.5 rounded-full tracking-wide">
                  Trainee
                </span>
              )}
            </div>
            <div className="flex items-center gap-2 mt-0.5">
              ...
              {post.subCommunity && (
                <a
                  ...
                  className="text-[10px] font-black text-emerald-600 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full hover:bg-emerald-100 transition-colors"
      ...
      {/* Content */}
      <div className="mb-4">
        <h2 className="text-lg font-bold text-black mb-1">{post.title}</h2>
        <p className="text-gray-700 text-sm leading-relaxed">{post.content}</p>
        {postImage && (
          <img src={postImage} className="w-full max-h-96 object-cover rounded-xl mt-3 border border-gray-100" />
        )}
      </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center bg-gray-100 rounded-full overflow-hidden border border-gray-200">
            <button
              ...
              className={`px-3 py-1.5 text-sm transition-colors ${myVote === 'up' ? 'text-emerald-600 bg-emerald-50' : 'text-gray-500 hover:bg-gray-200'}`}
            >
              ⬆️
            </button>
            <span className={`font-black text-sm px-2 min-w-[28px] text-center ${score > 0 ? 'text-emerald-600' : score < 0 ? 'text-red-500' : 'text-gray-600'}`}>
              {score}
            </span>
            <button
              onClick={() => handleVote('down')}
              className={`px-3 py-1.5 text-sm transition-colors ${myVote === 'down' ? 'text-red-500 bg-red-50' : 'text-gray-500 hover:bg-gray-200'}`}
            >
              ⬇️
            </button>
          </div>
          <button className="flex items-center gap-1.5 text-gray-500 hover:text-black transition-colors text-sm font-semibold">
        ...
        <button onClick={onReport} className="text-gray-400 hover:text-red-500 text-sm font-medium transition-colors">
          Report
        </button>
      </div>
    </div>
      ...
      <div className="w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-fadeInRight">
        <div className="p-4 border-b border-gray-200 flex items-center justify-between">
          <div>
            ...
            {groupName && (
              <p className="text-xs text-emerald-600 font-bold mt-0.5">📌 Posting in: {groupName}</p>
            )}
          </div>
        ...
        <div className="p-6 flex-1 overflow-y-auto space-y-5">
          {groupName && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-800">
              ⏳ Posts in sub-communities are reviewed by admins before appearing publicly.
            </div>
          )}

          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Topic Category</label>
            <select
          ...
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Attach Image (Optional)</label>
            ...
            {imageUrl ? (
              ...
              <div
                ...
                className="w-full border-2 border-dashed border-gray-300 rounded-xl p-8 text-center text-gray-500 text-sm hover:border-emerald-500 hover:bg-emerald-50 transition-colors cursor-pointer"
              >
      ...
      <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl">
        {done ? (
          ...
          <div className="p-8 text-center">
            ...
            <h2 className="text-lg font-black text-black mb-2">Report Submitted</h2>
          ...
          <>
            <div className="p-5 border-b border-gray-100">
              <h2 className="text-lg font-black text-black">Report Post</h2>
              <p className="text-xs text-gray-400 mt-0.5">Select the reason for reporting this post.</p>
            </div>
            <div className="p-5 space-y-2">
              {categories.map(cat => (
                <label key={cat} className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer border transition-colors ${selected === cat ? 'border-red-300 bg-red-50' : 'border-transparent hover:bg-gray-50 hover:border-gray-200'}`}>
                  <input
                    type="radio"
                    name="report"
                    value={cat}
            ...
            <div className="p-5 bg-gray-50 flex justify-end gap-3 border-t border-gray-100">
              <button onClick={onClose} className="px-4 py-2 text-sm font-bold text-gray-500 hover:text-black">
                Cancel
              </button>
              <button
...
const VoteMealView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <div className="p-6 animate-fadeIn">
      <button onClick={onBack} className="text-sm font-bold text-gray-500 hover:text-black mb-6">← Back to Forum</button>
      <h2 className="text-3xl font-black mb-2">Vote for Best Meal of the Week</h2>
      <p className="text-gray-500 mb-8">Support your fellow community members by voting for the most delicious and nutritious meal!</p>
      
      {loading ? (
        <div className="p-12 text-center text-gray-400">Loading...</div>
      ) : recipes.length === 0 ? (
        <div className="p-12 text-center bg-gray-50 rounded-2xl">No recipes submitted this week.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {recipes.map(recipe => (
            <div key={recipe.id} className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm flex flex-col hover:shadow-md transition-shadow">
              ...
              <div className="p-5 flex-1 flex flex-col">
                <h3 className="font-bold text-lg mb-1 text-gray-900">{recipe.recipeName}</h3>
                <p className="text-xs text-gray-600 mb-4 line-clamp-2">{recipe.instructions}</p>
                <div className="flex gap-3 text-xs font-bold text-gray-600 mb-4">
                  <span>🔥 {recipe.calories} kcal</span>
                  <span>🥩 {recipe.protein}g</span>
                </div>
                <div className="mt-auto flex items-center justify-between">
                  <span className="font-black text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">{votes[recipe.id] || 0} Votes</span>
                  <button 
